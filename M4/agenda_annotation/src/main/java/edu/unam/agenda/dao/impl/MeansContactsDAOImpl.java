...
public class MeansContactsDAOImpl implements MeansContactsDAO {
	...
	@Override
	public Boolean deleteAllMeansContactsByContactId(Integer id) {
		Boolean delete = Boolean.FALSE;
		try {
			Session session = HibernateUtil.getSessionFactory().openSession();
			// se inicia una transaccion
			session.beginTransaction();
			int deletedCount = session.createQuery("DELETE FROM MeansContacts t WHERE t.contact.id = :id")
					.setParameter("id", id)
					.executeUpdate();
			// se realiza el commit
			session.getTransaction().commit();
	...
	@Override
	public Set<MeansContacts> getAllMeansContactsByContactId(Integer id) {
		Set<MeansContacts> meansContactsSet = new HashSet<>();
		try {
			Session session = HibernateUtil.getSessionFactory().openSession();
			// se inicia una transaccion
			session.beginTransaction();
			List<MeansContacts> meansContactsList = session.createQuery("SELECT u FROM MeansContacts u WHERE u.contact.id = :id", MeansContacts.class)
					.setParameter("id", id)
					.getResultList();
			meansContactsSet.addAll(meansContactsList);
			// se realiza el commit
			session.getTransaction().commit();
			// se cierra la session hibernate
			session.close();
